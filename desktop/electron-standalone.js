...
function startBackend() {
  ...
  // When using asar packaging, backend is in resources/backend-standalone
  // When using electron-packager (no asar), backend is in resources/app/backend-standalone
  const isDev = !app.isPackaged;
  
  let backendExePath;
  if (isDev) {
    backendExePath = path.join(__dirname, 'backend-standalone', 'backend.exe');
  } else {
    // Try resources/backend-standalone first (asar packaging)
    backendExePath = path.join(process.resourcesPath, 'backend-standalone', 'backend.exe');
    // If not found, try resources/app/backend-standalone (electron-packager)
    if (!fs.existsSync(backendExePath)) {
      backendExePath = path.join(process.resourcesPath, 'app', 'backend-standalone', 'backend.exe');
    }
  }
  ...
  backendProcess = spawn(backendExePath, [], {
    env: {
      ...process.env,
      NODE_ENV: 'production',
      // DO NOT set USE_SQLITE - let backend use MySQL from .env.production
      PORT: '5000'
    },
