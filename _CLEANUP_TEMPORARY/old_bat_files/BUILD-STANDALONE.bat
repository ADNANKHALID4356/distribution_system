@echo off
echo.
echo ========================================================================
echo    Building Standalone Distribution Management System
echo ========================================================================
echo.
echo This process will:
echo   1. Build the React frontend
echo   2. Bundle the Node.js backend into a standalone executable
echo   3. Package everything into a Windows installer
echo.
echo This may take 10-15 minutes. Please wait...
echo.
pause

REM Step 1: Build React App
echo.
echo [1/6] Building React frontend application...
echo ----------------------------------------
cd desktop
call npm run build
if %errorlevel% neq 0 (
    echo.
    echo ERROR: React build failed!
    echo Please check the error messages above.
    pause
    exit /b 1
)
echo ✓ React frontend built successfully
cd ..

REM Step 2: Build backend standalone executable
echo.
echo [2/6] Building backend standalone executable...
echo ----------------------------------------
echo This will bundle Node.js with your backend...
cd backend
call npm run build-standalone
if %errorlevel% neq 0 (
    echo.
    echo ERROR: Backend standalone build failed!
    echo Please check the error messages above.
    pause
    exit /b 1
)
echo ✓ Backend executable created successfully
cd ..

REM Step 3: Verify backend executable exists
echo.
echo [3/6] Verifying backend executable...
echo ----------------------------------------
if not exist "desktop\backend-standalone\backend.exe" (
    echo.
    echo ERROR: Backend executable not found!
    echo Expected location: desktop\backend-standalone\backend.exe
    pause
    exit /b 1
)
echo ✓ Backend executable verified

REM Step 4: Update Electron configuration
echo.
echo [4/6] Preparing Electron configuration...
echo ----------------------------------------
cd desktop
if not exist "electron.js.backup" (
    copy /Y electron.js electron.js.backup
)
copy /Y electron-standalone.js electron.js
echo ✓ Electron configuration updated

REM Step 5: Install/verify electron-builder
echo.
echo [5/6] Preparing electron-builder...
echo ----------------------------------------
call npm list electron-builder >nul 2>&1
if %errorlevel% neq 0 (
    echo Installing electron-builder (this may take a few minutes)...
    call npm install electron-builder --save-dev
    if %errorlevel% neq 0 (
        echo ERROR: Failed to install electron-builder
        pause
        exit /b 1
    )
)
echo ✓ electron-builder ready

REM Step 6: Build the standalone installer
echo.
echo [6/6] Building Windows installer...
echo ----------------------------------------
echo This is the longest step - creating the installer package...
echo.
call npx electron-builder build --win --config package-standalone.json
if %errorlevel% neq 0 (
    echo.
    echo ERROR: Installer build failed!
    pause
    exit /b 1
)

REM Restore original electron.js
if exist "electron.js.backup" (
    copy /Y electron.js.backup electron.js
    del electron.js.backup
)

echo.
echo ========================================================================
echo    BUILD COMPLETED SUCCESSFULLY!
echo ========================================================================
echo.
echo Your standalone installer is ready:
echo.
echo Location: desktop\dist-standalone\
echo File: Distribution Management System-Setup-1.0.0.exe
echo.
echo This installer includes:
echo   ✓ React Frontend Application
echo   ✓ Node.js Backend Server
echo   ✓ SQLite Database (auto-created on first run)
echo   ✓ All dependencies bundled
echo.
echo Installation instructions:
echo   1. Copy the installer to any Windows PC
echo   2. Run the installer as administrator
echo   3. Follow the installation wizard
echo   4. Launch the application from desktop shortcut
echo   5. Default login: admin / admin123
echo.
echo The application will create its database in:
echo   %APPDATA%\DistributionSystem\
echo.
pause
